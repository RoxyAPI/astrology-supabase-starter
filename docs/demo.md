...
It calls the two functions with the project publishable key, sent on the `apikey` header, which both require,
and renders the results with the published web components from the CDN, so the markup stays small and the
reading looks the same as it does anywhere else those components are used. It never signs in, so it shows both
readings and saves no chart.
